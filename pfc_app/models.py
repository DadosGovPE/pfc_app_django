...
class PesquisaCursosPriorizados(models.Model):
    ...
    forma_atendimento = models.CharField(
        max_length=10, choices=FORMA_CHOICES, blank=True, null=True
    )
    trilha = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="pesquisa_cursos_priorizados",
    )
...
class Lotacao(models.Model):
    ...
    def get_abreviacao_lotacao(self):
        ...
        if len(nome_lotacao) > 13:
            ...
            palavras_filtradas = [
                palavra for palavra in palavras if palavra.lower() not in preposicoes
            ]
...
class LotacaoEspecifica(models.Model):
    ...
    lotacao = models.ForeignKey(
        Lotacao, on_delete=models.CASCADE, related_name="especificacoes"
    )
...
class User(AbstractUser):
    ...
    lotacao_especifica_2 = models.CharField(
        max_length=400, blank=True, null=True, verbose_name=("Lotação sigla")
    )
    lotacao_fk = models.ForeignKey(
        Lotacao,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    ...
    lotacao_especifica_fk = models.ForeignKey(
        LotacaoEspecifica,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="users_especifica",
        verbose_name=("Lotação Específica"),
    )
    classificacao_lotacao = models.CharField(max_length=400, blank=True, null=True)
    pesquisa_cursos_priorizados = models.ManyToManyField(
        PesquisaCursosPriorizados, blank=True
    )
    carreira = models.ForeignKey(
        Carreira, on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    is_primeiro_acesso = models.BooleanField(
        default=False, verbose_name=("É o primeiro acesso")
    )
...
class PriorizacaoResposta(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="priorizacao_respostas"
    )
...
class Trilha(models.Model):
    ...
    ordem_relatorio = models.PositiveSmallIntegerField(
        blank=True, null=True, verbose_name=("Ordem no relatório")
    )
...
class CursoPriorizado(models.Model):
    ...
    trilha = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="cursos_priorizados",
    )
...
class Curso(models.Model):
    ...
    nome_curso = models.CharField(
        max_length=400, blank=False, null=False, db_index=True
    )
    ementa_curso = models.TextField(max_length=4000, blank=False, null=False)
    modalidade = models.ForeignKey(
        Modalidade, on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    inst_certificadora = models.ForeignKey(
        InstituicaoCertificadora, on_delete=models.SET_NULL, blank=True, null=True
    )
    inst_promotora = models.ForeignKey(
        InstituicaoPromotora, on_delete=models.SET_NULL, blank=True, null=True
    )
    participantes = models.ManyToManyField(
        User, through="Inscricao", related_name="curso_participante"
    )
    avaliacoes = models.ManyToManyField(
        User, through="Avaliacao", related_name="curso_avaliacao"
    )
    avaliacoes_abertas = models.ManyToManyField(
        User, through="AvaliacaoAberta", related_name="avaliacoes_abertas"
    )
    ...
    trilha = models.ForeignKey(
        Trilha, on_delete=models.PROTECT, blank=True, null=True, related_name="cursos"
    )
    ...
    observacao = models.TextField(
        max_length=4000, blank=True, null=True, verbose_name=("Observação")
    )
    horario = models.TextField(
        max_length=4000, blank=True, null=True, verbose_name=("Horário")
    )
    curso_priorizado = models.ForeignKey(
        CursoPriorizado,
        on_delete=models.SET_NULL,
        blank=True,
        null=True,
    ...
    origem_pagamento = models.ForeignKey(
        OrigemPagamento,
        on_delete=models.SET_NULL,
        blank=True,
        null=True,
        related_name="cursos_origem",
    )
...
class ArquivoCurso(models.Model):
    ...
    def __str__(self):
        return (
            self.descricao
            if self.descricao
            else f"Arquivo do curso {self.curso.nome_curso}"
        )
...
class PlanoCurso(models.Model):
    ...
    publico_alvo = models.TextField(
        max_length=4000, blank=True, null=True, verbose_name=("Público-alvo")
    )
    quantidade_turma = models.SmallIntegerField(
        blank=True, null=True, verbose_name=("Quantidade de turmas")
    )
    pre_requisitos = models.TextField(
        max_length=4000, blank=True, null=True, verbose_name=("Pré-requisitos")
    )
    objetivo_geral = models.TextField(
        max_length=4000, blank=True, null=True, verbose_name=("Objetivo geral")
    )
    objetivo_especifico = models.TextField(
        max_length=4000, blank=True, null=True, verbose_name=("Objetivo específico")
    )
    metodologia_ensino = models.TextField(
        max_length=4000, blank=True, null=True, verbose_name=("Metodologia de ensino")
    )
    metodologia_avaliacao = models.TextField(
        max_length=4000,
        blank=True,
        null=True,
        verbose_name=("Metodologia de avaliação"),
    )
    recursos_professor = models.TextField(
        max_length=4000, blank=True, null=True, verbose_name=("Recursos professor")
    )
    recursos_aluno = models.TextField(
        max_length=4000, blank=True, null=True, verbose_name=("Recursos aluno")
    )
    referencia_bibliografica = models.TextField(
        max_length=4000,
        blank=True,
        null=True,
        verbose_name=("Referências bibliográficas"),
    )
...
class CronogramaExecucao(models.Model):
    ...
    conteudo = models.TextField(
        max_length=4000, blank=True, null=True, verbose_name=("Conteúdo")
    )
    atividade = models.TextField(
        max_length=4000, blank=True, null=True, verbose_name=("Atividades")
    )
...
class Inscricao(models.Model):
    ...
    condicao_na_acao = models.CharField(
        max_length=400,
        choices=CONDICAO_ACAO_CHOICES,
        blank=False,
        null=False,
        default="DISCENTE",
    )
...
class Avaliacao(models.Model):
    ...
    subtema = models.ForeignKey(
        Subtema, blank=False, null=False, on_delete=models.CASCADE
    )
...
class Curadoria(models.Model):
    ...
    instituicao_promotora = models.ForeignKey(
        InstituicaoPromotora, on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    curso_priorizado = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="curadorias_priorizadas",
    )
...
class Validacao_CH(models.Model):
    ...
    usuario = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="requerente_validacao"
    )
    ...
    requerimento_ch = models.ForeignKey(
        "RequerimentoCH", on_delete=models.SET_NULL, blank=True, null=True
    )
    responsavel_analise = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="responsavel_validacao",
    )
    ...
    curadoria = models.ForeignKey(
        Curadoria, on_delete=models.SET_NULL, blank=True, null=True
    )
    agenda_pfc = models.BooleanField(default=False, blank=False, null=False)
    status = models.ForeignKey(
        StatusValidacao, on_delete=models.DO_NOTHING
    )  # , default=status_validacao.id)
    condicao_na_acao = models.CharField(
        max_length=20,
        choices=CONDICAO_ACAO_CHOICES,
        blank=False,
    ...
    carreira = models.ForeignKey(
        Carreira, on_delete=models.DO_NOTHING, blank=True, null=True
    )
    trilha = models.ForeignKey(
        Trilha, on_delete=models.DO_NOTHING, blank=True, null=True
    )
    ...
    )  # Armazena o ano da validação
    numero_sequencial = models.IntegerField(
        blank=True, null=True
    )  # Número que será reiniciado a cada ano
    ...
    def get_proximo_numero(self):
        ...
        ultimo = (
            Validacao_CH.objects.filter(ano=self.ano, numero_sequencial__isnull=False)
...
class Certificado(models.Model):
    ...
    assinatura = models.FileField(
        upload_to="upload/certificado/",
        blank=True,
        null=True,
        default="upload/certificado/",
    )
