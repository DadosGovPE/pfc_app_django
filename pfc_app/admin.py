...
class InscricaoInline(admin.TabularInline):
    ...
    def get_queryset(self, request):
        ...
        return queryset.select_related("participante").only(
            "participante__id", "condicao_na_acao", "status"
        )
...
class ParticipanteNomeFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return [
            (participante.id, participante.nome) for participante in User.objects.all()
        ]
