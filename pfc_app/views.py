...
def dashboard(request):
    ...
    cursos_qs = (
        Curso.objects.annotate(
            ano=ExtractYear("data_inicio"), mes=ExtractMonth("data_inicio")
        )
        .values("ano", "mes")
    ...
    inscricoes_qs = (
        Inscricao.objects.annotate(
            ano=ExtractYear("curso__data_inicio"),
            mes=ExtractMonth("curso__data_inicio"),
        )
        .values("ano", "mes")
        .annotate(total=Count("id"))
    )
    ...
    context = {
        ...
        "cursos_por_ano_mes": json.dumps(
            {ano: cursos_por_ano_mes[ano] for ano in anos}
        ),
        "inscricoes_por_ano_mes": json.dumps(
            {ano: inscricoes_por_ano_mes[ano] for ano in anos}
        ),
    }
...
@csrf_exempt  # <-- evita checagem de CSRF
@require_POST
...
def registrar(request):
    ...
    if not re.match(nome_regex, nome):
        messages.error(
            request,
            f"{nome}, por favor, insira um nome válido (não são aceitos nomes com apenas números).",
        )
    ...
    if UserCadastro.objects.filter(cpf=cpf).exists():
        messages.error(
            request, f"❌ Atenção você foi bloqueado por múltiplas tentativas!"
        )
        return render(request, "pfc_app/registrar.html", context)
    ...
    messages.success(
        request, f"Solicitação enviada com sucesso. Aguarde suas credenciais!"
    )
...
def cursos(request):
    ...
    cursos_com_inscricoes = (
        Curso.objects.annotate(
            num_inscricoes=Count(
                "inscricao",
            ...
            usuario_inscrito=Exists(
                Inscricao.objects.filter(
                    participante=request.user, curso=OuterRef("pk")
                )
            ),
        ...
        .all()
        .filter(data_inicio__gte=data_atual)
...
def usuarios_sem_ch(request):
    ...
    inscricoes = Inscricao.objects.filter(
        participante=OuterRef("pk"), concluido=True, curso__status__nome="FINALIZADO"
    )

    if data_inicio:
        inscricoes = inscricoes.filter(
            curso__data_termino__gte=data_inicio
    ...
    if data_inicio:
        validacoes = validacoes.filter(
            data_termino_curso__gte=data_inicio
        )  # Substitua 'data' pelo campo correto
    if data_fim:
        validacoes = validacoes.filter(
            data_termino_curso__lte=data_fim
        )  # Substitua 'data' pelo campo correto

    validacoes = (
        validacoes.values("usuario")
        .annotate(total_ch=Sum("ch_confirmada"))
        .values("total_ch")[:1]
    )
    ...
    users = (
        ...
        .annotate(total_ch=F("total_ch_inscricao") + F("total_ch_validacao"))
        .filter(grupo_ocupacional="GGOV")
        .order_by("nome")
    )
...
def user_cadastro(request):
    ...
    if form.is_valid():
        ...
        if data_inicio and data_fim:
            lista_usuarios = UserCadastro.objects.filter(
                data_solicitacao__gte=data_inicio, data_solicitacao__lte=data_fim
            )
    ...
    context = {
        ...
        "values": request.GET
        if request.GET
        else {"data_inicio": data_hoje, "data_fim": data_hoje},
...
def processar_checkboxes(request):
    ...
    if request.method == "POST":
        ...
        if not usuarios_selecionados:
            ...
            return redirect(
                f"{reverse('user_cadastro')}?data_inicio={data_inicio}&data_fim={data_fim}"
            )
        ...
        for usuario_cpf in usuarios_selecionados:
            ...
            try:
                ...
                messages.error(
                    request, f"Usuário com CPF {usuario_cpf} não encontrado."
                )
                continue
        ...
        if usuarios_criados:
            messages.success(
                request, f"Usuários criados com sucesso: {', '.join(usuarios_criados)}."
            )
...
def carga_horaria(request):
    ...
    if request.user.role == "ADMIN" or request.user.is_superuser:
        ...
        if usuarios is None:
            ...
            cache.set(
                "lista_usuarios", usuarios, timeout=60 * 60 * 24
            )  # guarda por 24h
    ...
    try:
        ...
        status_validacao_deferida_parc = StatusValidacao.objects.get(
            nome="DEFERIDA PARCIALMENTE"
        )
    ...
    validacoes = Validacao_CH.objects.filter(
        ...
        status__in=[status_validacao_deferida, status_validacao_deferida_parc],
    )
    validacoes_indeferidas = Validacao_CH.objects.filter(
    ...
    carga_horaria_pfc = (
        inscricoes_do_usuario.aggregate(Sum("ch_valida"))["ch_valida__sum"] or 0
    )
    validacoes_ch = (
...
class CursoDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        usuario_inscrito = Inscricao.objects.filter(
            curso=curso, participante=self.request.user
        ).exists()
...
def avaliacao(request, curso_id):
    ...
    if request.method == "POST":
        ...
        avaliacao_aberta = AvaliacaoAberta(
            curso=curso,
            participante=request.user,
...
def validar_ch(request):
    ...
    if request.method == "POST":
        ...
        validacao = Validacao_CH(
            usuario=request.user,
            arquivo_pdf=arquivo_pdf,
            nome_curso=nome_curso,
            ch_solicitada=ch_solicitada,
            data_termino_curso=data_termino,
            data_inicio_curso=data_inicio,
        ...
        validacao.arquivo_pdf.name = os.path.join(
            "uploads", request.user.username, novo_nome_arquivo
        )
        validacao.save()
        ...
        for user in users:
            ...
            try:
                inscricao = Inscricao.objects.get(
                    participante=user, curso=curso, concluido=True
                )
            except:
                continue
...
def generate_single_pdf(request, inscricao_id):
    ...
    try:
        ...
        messages.error(
            request, f"CPF de {user.nome} está com número de caracteres errado!"
        )
        return redirect("lista_cursos")
    ...
    pdf_filename = os.path.join(
        output_folder, f"{user.username}-{curso.nome_curso}.pdf"
    )
    # Crie o PDF usando ReportLab
...
def generate_all_reconhecimento(request, validacao_id):
    ...
    with zipfile.ZipFile(zip_filename, "w", zipfile.ZIP_DEFLATED) as zipf:
        try:
            ...
            # Formata o CPF no formato "000.000.000-00"
            cpf_formatado = (
                f"{user.cpf[:3]}.{user.cpf[3:6]}.{user.cpf[6:9]}-{user.cpf[9:]}"
            )
        except:
            messages.error(
                request, f"CPF de {user.nome} está com número de caracteres errado!"
        ...
        if competencias:  # Verifica se a lista não está vazia
            ...
            texto_competencia = (
                ", ".join(competencias[:-1]) + " e " + competencias[-1]
        ...
        pdf_filename = os.path.join(
            output_folder,
            f"{validacao.numero_sequencial}-{user.username}-requerimento.pdf",
        )
        # Crie o PDF usando ReportLab

        style_body = ParagraphStyle(
            "body", fontName="Helvetica", fontSize=12, leading=17, alignment=TA_JUSTIFY
        )
        style_rodape = ParagraphStyle(
            "rodape", fontName="Helvetica", fontSize=12, leading=17, alignment=TA_CENTER
        )
        style_title = ParagraphStyle(
            "title", fontName="Helvetica", fontSize=16, alignment=TA_CENTER
        )
        style_subtitle = ParagraphStyle("subtitle", fontName="Helvetica", fontSize=14)
        style_numero = ParagraphStyle(
            "numero", fontName="Helvetica", fontSize=12, leading=17, alignment=TA_RIGHT
        )
        ...
        numero_doc = (
            "N° "
            + str(validacao.numero_sequencial)
            + "/"
            + str(validacao.analisado_em.year)
        )
        print(numero_doc)
        ...
        if form.is_valid():
            ...
            if user:
                ...
                try:
                    ...
                    messages.error(
                        request, "Erro no BD inesperado, entre em contato com o IG!"
                    )
                ...
                try:
                    send_mail(
                        "Senha Nova - AppPFC",
                        f"Sua nova senha é: {senha_aleatoria}",
                        "ncdseplag@gmail.com",
                        [user.email],
                    )
                except:
                    ...
                    user.save()
                    messages.error(
                        request, "Erro no envio de email. Sua nova senha é: 12345678"
                    )
                    return redirect("lista_cursos")
            else:
...
def draw_logos_infos(curso, canvas, doc):
    ...
    logo_height = 50
    x_ig = (
        doc.width + doc.leftMargin + doc.rightMargin - 27.5 - logo_width
    ...
    required_height = paragraph.wrap(max_width, 40)[
        1
...
def draw_logos(curso, canvas, doc):
    ...
    canvas.drawImage(
        pfc_path, x_pfc, y_pfc, width=logo_width, height=logo_height, mask="auto"
    )
...
def assinatura_ata(curso):
    ...
    signature_style.alignment = (
        1  # Centraliza o texto (0 = esquerda, 1 = centro, 2 = direita)
    )
    ...
    if not curso.eh_evento:
        ...
        for i, docente in enumerate(docentes):
            ...
            if i < len(docentes) - 1:  # Verifica se não é o último instrutor
                instrutor_elements.append([Paragraph("", signature_style)] * 4)
...
def gerar_ata(request, curso_id):
    ...
    lista_inscritos = curso.participantes.filter(
        inscricao__condicao_na_acao="DISCENTE", inscricao__status__nome="APROVADA"
    ).order_by("nome")
    # Cabeçalho da tabela
...
def capa_relatorio(c: canvas.Canvas, width, height, plano_curso: PlanoCurso):
    ...
    c.drawString(
        x_position - (text_width / 2), y_position - 40, plano_curso.curso.nome_curso
    )
...
def pagina_dados_curso(c: canvas.Canvas, width, height, curso_id):
    ...
    c.drawString(
        x_position,
        y_position,
        "Data de início: " + str(data_inicio.strftime("%d/%m/%Y")),
    )
    y_position = y_position - 20
    c.drawString(
        x_position,
        y_position,
        "Data de término: " + str(data_termino.strftime("%d/%m/%Y")),
    )
    y_position = y_position - 20
...
def gerar_relatorio(request, curso_id):
    ...
    avaliacoes = Avaliacao.objects.filter(curso_id=curso_id).select_related(
        "subtema", "subtema__tema"
    )
    medias_notas_por_tema = (
        Avaliacao.objects.filter(~Q(nota="0"), curso_id=curso_id)
        .annotate(nota_numerica=Cast("nota", FloatField()))
    ...
    quantidade_avaliadores = (
        Avaliacao.objects.filter(curso_id=curso_id)
        .values("participante")
        .distinct()
        .count()
    )
    ...
    colors = ["#F8D9A3", "#FFD559", "#DDDDDD", "#B7E6E1", "#86D4CD", "#76C2AA"]
    width, height = A4
    pagina_dados_curso(c, width, height, curso_id)

    # http://127.0.0.1:8000/gerar_relatorio/13
    for tema in temas:
        total_avaliacoes_tema = Avaliacao.objects.filter(
            subtema__tema_id=tema.id
        ).count()
        avaliacoes_baixas_tema = Avaliacao.objects.filter(
            subtema__tema_id=tema.id, nota__in=["1", "2"]
        ).count()
        percentual_baixas_tema = (
            (avaliacoes_baixas_tema / total_avaliacoes_tema) * 100
            if total_avaliacoes_tema > 0
            else 0
        )
        percentual_baixas_tema = round(percentual_baixas_tema, 2)

        total_avaliacoes_zero = Avaliacao.objects.filter(
            subtema__tema_id=tema.id, nota="0"
        ).count()

        avaliacoes_altas_tema = Avaliacao.objects.filter(
            subtema__tema_id=tema.id, nota__in=["4", "5"]
        ).count()
        percentual_altas_tema = (
            (avaliacoes_altas_tema / (total_avaliacoes_tema - total_avaliacoes_zero))
            * 100
            if total_avaliacoes_tema > 0
            else 0
        )
        ...
        media_notas = medias_notas_dict.get(
            tema.id, 0
        )  # Obtém a média das notas para o tema ou 0 se não houver avaliações
        ...
        for subtema, notas in notas_por_subtema.items():
            ...
            cor_subtema = {cor: cor}
            proporcoes = {
                nota: (count / total if total else 0) for nota, count in notas.items()
            }
            ...
            if tema.nome == "Conhecimento":
                if subtema == "Conhecimento Prévio":
                    ...
                    media_conhecimento_previo = round(
                        soma_notas_vezes_count / soma_counts, 1
                    )
                    ...
                    # print(soma_counts)
                    # print(soma_notas_vezes_count/soma_counts)
                    media_conhecimento_posterior = round(
                        soma_notas_vezes_count / soma_counts, 1
                    )
        ...
        for i, nota in enumerate(range(0, 6)):
            valores = [
                proporcoes_por_subtema[subtema].get(str(nota), 0)
                for subtema in proporcoes_por_subtema
            ]
        ...
        if len(subtema_names) == 1:
            ax.set_ylim(
                -1, 1
            )  # Ajuste estes valores conforme necessário para manter a altura das barras
        else:
        ...
        c.drawImage(
            f"{tema.id}_proporcoes.png", 50, 200, width=500, height=250
        )  # Ajuste as dimensões conforme necessário
        # http://127.0.0.1:8000/gerar_relatorio/12
        width_rect = 100
        height_rect = 70
        c.roundRect(
            (width / 2) + 50, 470, width_rect, height_rect, 10, stroke=1, fill=0
        )
        c.roundRect(
            (width / 2) - 150, 470, width_rect, height_rect, 10, stroke=1, fill=0
        )
        media_text = "Média Geral"
        if tema.nome == "Conhecimento":
            media_conhecimento = round(
                media_conhecimento_posterior / media_conhecimento_previo, 1
            )
        ...
        kpi_numero_text_width, kpi_numero_text_height = kpi_numero_text.wrapOn(
            c, 80, 40
        )
        ...
        texto_y = (
            470 + (height_rect / 2) - 6
        )  # Ajuste o valor para centralizar verticalmente
        texto_x2 = (width / 2) - 150 + (width_rect / 2) - (kpi_numero_width / 2)
        texto_y2 = 470 + (height_rect / 2) - 6
        kpi_text_numero_x = (
            (width / 2) - 150 + (width_rect / 2) - (kpi_numero_text_width / 2)
        )
        kpi_text_numero_y = 470 + height_rect - kpi_numero_text_height
        kpi_text_media_x = (
            (width / 2) + 50 + (width_rect / 2) - (kpi_media_text_width / 2)
        )
...
def gerar_curadoria(request, ano, mes):
    ...
    cabecalho = [
        Paragraph("Curadoria", header_style),
        ...
        for curso in trilha.cursos.filter(
            ...
            data_pfc.append(
                [
                    ...
                    Paragraph(
                        f"de {curso.data_inicio.strftime('%d/%m/%Y')} a {curso.data_termino.strftime('%d/%m/%Y')}",
                        body_style,
                    ),
                ],
            )
        ...
        for curadoria in curadorias:
            ...
            data.append(
                [
                    ...
                    Paragraph(
                        ""
                        if curadoria.instituicao_promotora is None
                        else curadoria.instituicao_promotora.nome,
                        body_style,
                    ),
                ],
            )
...
def curadoria(request):
    ...
    if Curadoria.objects.exists():
        ...
        min_year_curadoria = (
            year_range_curadoria["min_year"]
            if year_range_curadoria["min_year"] is not None
            else 0
        )
        max_year_curadoria = (
            year_range_curadoria["max_year"]
            if year_range_curadoria["max_year"] is not None
            else 0
        )
    else:
    ...
    if Curso.objects.exists():
        ...
        min_year_pfc = (
            year_range_pfc["min_year"] if year_range_pfc["min_year"] is not None else 0
        )
        max_year_pfc = (
            year_range_pfc["max_year"] if year_range_pfc["max_year"] is not None else 0
        )
    else:
...
def curadoria_html_show(request):
    ...
    if Curadoria.objects.exists():
        ultima_data = Curadoria.objects.aggregate(ultima=Max("mes_competencia"))[
            "ultima"
        ]
        if ultima_data:
...
def estatistica_lnt(request):
    ...
    todos_anos = CursoPriorizado.objects.dates(
        "mes_competencia", "year", order="ASC"
    ).distinct()

    total_cursos = Curso.objects.filter(Q(data_inicio__year=ano_referencia)).count()
    total_curadorias = Curadoria.objects.filter(
        Q(mes_competencia__year=ano_referencia)
    ).count()
    total_cursos_ofertados = total_cursos + total_curadorias
    total_cursos_priorizados = CursoPriorizado.objects.filter(
        Q(mes_competencia__year=ano_referencia)
    ).count()

    cursos_priorizados_count = Curso.objects.filter(
        curso_priorizado__isnull=False, data_inicio__year=ano_referencia
    ).count()
    curadorias_priorizadas_count = Curadoria.objects.filter(
        curso_priorizado__isnull=False, mes_competencia__year=ano_referencia
    ).count()
    cursos_priorizados_ids = Curso.objects.filter(
        curso_priorizado__isnull=False, data_inicio__year=ano_referencia
    ).values_list("curso_priorizado_id", flat=True)
    curadorias_priorizadas_ids = Curadoria.objects.filter(
        curso_priorizado__isnull=False, mes_competencia__year=ano_referencia
    ).values_list("curso_priorizado_id", flat=True)
    cursos_nao_ofertados_count = CursoPriorizado.objects.filter(
        ~Q(id__in=cursos_priorizados_ids)
        & ~Q(id__in=curadorias_priorizadas_ids)
        & Q(mes_competencia__year=ano_referencia)
    ).count()
    ...
    cursos_priorizados_percent = (
        (cursos_priorizados_count / total_cursos * 100) if total_cursos else 0
    )
    curadorias_priorizadas_percent = (
        (curadorias_priorizadas_count / total_curadorias * 100)
        if total_curadorias
        else 0
    )
    total_ofertado_percent = (
        (
            (total_cursos_priorizados - cursos_nao_ofertados_count)
            / total_cursos_priorizados
            * 100
        )
        if total_cursos_priorizados
        else 0
    )
...
def cursos_mais_votados(request):
    ...
    if request.method == "POST":
        ...
        cursos_priorizados = CursoPriorizado.objects.filter(
            mes_competencia=data_referencia
        ).values_list("nome_sugestao_acao", flat=True)
        ...
        for curso_id in cursos_selecionados_ids:
            ...
            if curso.nome not in cursos_priorizados:
                CursoPriorizado.objects.create(
                    nome_sugestao_acao=curso.nome, mes_competencia=mes_ref
                )

        # Remover cursos que foram desmarcados e estão na tabela CursoPriorizado
        cursos_a_remover = PesquisaCursosPriorizados.objects.exclude(
            id__in=cursos_selecionados_ids
        )
    ...
    cursos_votados = (
        PesquisaCursosPriorizados.objects.filter(ano_ref=ano_referencia_pesquisa)
        .annotate(num_votos=Count("user"))
        .order_by("-num_votos")
    )

    # Obter os IDs dos cursos já priorizados
    cursos_priorizados_nomes = CursoPriorizado.objects.filter(
        mes_competencia=data_referencia
    ).values_list("nome_sugestao_acao", flat=True)
    cursos_priorizados_ids = PesquisaCursosPriorizados.objects.filter(
        ano_ref=ano_referencia_pesquisa, nome__in=cursos_priorizados_nomes
    ).values_list("id", flat=True)

    context = {
        "ano_referencia_pesquisa": ano_referencia_pesquisa,
        "cursos_votados": cursos_votados,
        "cursos_priorizados_ids": list(
            cursos_priorizados_ids
        ),  # Convertendo para lista para usar no template
    }
...
def generate_bda_pdf(request, ano, mes):
    ...
    cursos = Curso.objects.filter(
        data_inicio__month=mes_referencia,
        data_inicio__year=ano_referencia,
        curso_priorizado__isnull=False,
    ).exclude(status=status_curso)
    curadorias = Curadoria.objects.filter(
        mes_competencia__month=mes_referencia,
        mes_competencia__year=ano_referencia,
        curso_priorizado__isnull=False,
    )
    ...
    pdf_path = os.path.join(
        settings.BASE_DIR, "pdf_output", "comprovante_bda_{ano}.pdf"
    )
    doc = SimpleDocTemplate(
        pdf_path, pagesize=A4, title=f"Comprovante BDA IGPE {ano_referencia}"
    )
    ...
    table_cell_style = ParagraphStyle(
        name="Table_Cell", fontSize=10, leading=12, wordWrap="CJK"
    )
    header_cell_style = ParagraphStyle(
        name="Header_Cell", fontSize=10, leading=12, alignment=1, wordWrap="CJK"
    )  # Centralizado
    # Elementos do PDF
    elements = []
    ...
    for curadoria in curadorias:
        data.append(
            [
                Paragraph(
                    curadoria.curso_priorizado.nome_sugestao_acao, table_cell_style
                ),
        ...
        TableStyle(
            [
                ...
                (
                    ...
                    (-1, -1),
                    "TOP",
                ),  # Alinha o texto ao topo da célula para as linhas de dados
            ]
        )
    )
    ...
    stats_header = [
        ...
        Paragraph(
            "% LNT incremental do mês (considerando apenas cursos não atendidos)",
            header_cell_style,
        ),
    ...
    for month in range(1, mes_referencia + 1):
        cursos_priorizados_ofertados = CursoPriorizado.objects.filter(
            Q(curadorias_priorizadas__mes_competencia__month=month)
            | Q(cursos_priorizados__data_inicio__month=month),
            mes_competencia__year=ano_referencia,
        ).distinct()

        cursos_novos_ofertados = cursos_priorizados_ofertados.exclude(
            pk__in=cursos_priorizados_jacontabilizados
        )

        # Atualiza o conjunto de cursos já contabilizados
        cursos_priorizados_jacontabilizados.update(
            cursos_novos_ofertados.values_list("pk", flat=True)
        )

        percentage_ofertados = (
            (cursos_priorizados_ofertados.count() / total_cursos_priorizados) * 100
            if total_cursos_priorizados
            else 0
        )
        percentage_novos = (
            (cursos_novos_ofertados.count() / total_cursos_priorizados) * 100
            if total_cursos_priorizados
            else 0
        )
    ...
    # Gera o PDF
    # doc.build(elements)
    doc.build(elements, onFirstPage=lambda canvas, doc: draw_igpe_logo(canvas, doc))
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="comprovante_bda_{ano}.pdf"'
    )
...
def duplicar_plano_curso(request):
    ...
    planos = sorted(
        ...
        key=lambda p: (
            p.curso.data_termino.year if p.curso.data_termino else 0,
            p.curso.nome_curso,
        ),
        reverse=True,
    )
...
def estatistica_bda(request):
    ...
    cursos_priorizados_ids = (
        Curso.objects.filter(
            curso_priorizado__isnull=False, data_inicio__year=current_year
        )
        .exclude(status__nome__in=["CANCELADO", "A INICIAR"])
        .values_list("curso_priorizado_id", flat=True)
    )
    curadorias_priorizadas_ids = Curadoria.objects.filter(
        curso_priorizado__isnull=False, mes_competencia__year=current_year
    ).values_list("curso_priorizado_id", flat=True)
    print(len(curadorias_priorizadas_ids))
    cursos_nao_ofertados_count = CursoPriorizado.objects.filter(
        ~Q(id__in=cursos_priorizados_ids)
        & ~Q(id__in=curadorias_priorizadas_ids)
        & Q(mes_competencia__year=current_year)
    ).count()
    total_cursos_priorizados = CursoPriorizado.objects.filter(
        Q(mes_competencia__year=current_year)
    ).count()
    ...
    if CursoPriorizado.objects.exists():
        ...
        # min_year_curadoria
        min_year_priorizados = (
            year_range_priorizados["min_year"]
            if year_range_priorizados["min_year"] is not None
            else 0
        )
        max_year_priorizados = (
            year_range_priorizados["max_year"]
            if year_range_priorizados["max_year"] is not None
            else 0
        )
    else:
...
def get_nova_lotacao_especifica(request):
    ...
    context = {"nova_lotacoes_especificas": especificacoes}
...
def gastos(request):
    ...
    cursos = Curso.objects.filter(origem_pagamento__isnull=False).annotate(
        instrutores_count=Count(
            "inscricao", filter=Q(inscricao__condicao_na_acao="DOCENTE")
        ),
        valor_instrutor_primario=Case(
            When(origem_pagamento__nome="Sem Pagamento", then=Value(0)),
            default=ExpressionWrapper(
                Subquery(latest_ajuste_subquery.values("valor_instrutor_primario"))
                * F("ch_curso"),
            ...
            default=ExpressionWrapper(
                Case(
                    When(
                        ...
                        then=Subquery(
                            latest_ajuste_subquery.values("valor_instrutor_secundario")
                        ),
                    ),
                    default=Value(0),
                    output_field=DecimalField(),
        ...
        valor_coordenador=Case(
            When(origem_pagamento__nome="Sem Pagamento", then=Value(0)),
            default=ExpressionWrapper(
                Subquery(latest_ajuste_subquery.values("valor_coordenador"))
                * F("ch_curso"),
                output_field=DecimalField(),
            ),
            output_field=DecimalField(),
    ...
    total_gasto_atual_ano = (
        cursos.filter(data_inicio__year=current_year).aggregate(
...
def carga_horaria_por_cpf(request):
    ...
    try:
        ...
        inscricoes_qs = (
            ...
            .annotate(
                ch_por_curso=Max("ch_valida")
            )  # pega a CH por curso (se houver duplicadas, usa a maior)
        )
...
def buscar_parlamentares(request):
    ...
    try:
        ...
        df_filtrado = df[
            df["parlamentar_normalizado"].str.contains(nome_normalizado, na=False)
        ]
        ...
        return JsonResponse(
            {"resultados": parlamentares_unicos.to_dict(orient="records")}
        )
...
def buscar_municipios(request):
    ...
    try:
        ...
        # Filtra onde o nome buscado está contido
        df_filtrado = df[
            df["municipio_normalizado"].str.contains(nome_normalizado, na=False)
        ]

        # Remove duplicados
        municipios_unicos = df_filtrado[
            ["municipio_normalizado", "CD_MUNICIPIO"]
        ].drop_duplicates()
        municipios_unicos["municipio_normalizado"] = municipios_unicos[
...
def resumo_emendas(request, id_parlamentar):
    ...
    impedimentos = df_filtrado[
        df_filtrado["IMPEDIMENTO TÉCNICO"].fillna("").str.upper() == "SIM"
    ].shape[0]
    ...
    return JsonResponse(
        {
            "nome": nome,
            "investimento_total": f"R$ {investimento_total:,.2f}".replace(",", "X")
            .replace(".", ",")
            .replace("X", "."),
            "liquidado_total": f"R$ {liquidado_total:,.2f}".replace(",", "X")
            .replace(".", ",")
            .replace("X", "."),
...
def resumo_emendas_municipio(request, cd_mun: int):
    ...
    if not os.path.exists(caminho_csv):
        return JsonResponse(
            {"erro": "Arquivo emendas.csv não encontrado em /media"}, status=404
        )
    ...
    investimento_total = (
        df_filtrado[col_inv].apply(parse_valor).sum()
        if col_inv in df_filtrado.columns
        else 0.0
    )
    liquidado_total = (
        df_filtrado[col_liq].apply(parse_valor).sum()
        if col_liq in df_filtrado.columns
        else 0.0
    )
    impedimentos = (
        df_filtrado[col_imp].fillna("").astype(str).str.upper().eq("SIM").sum()
        if col_imp in df_filtrado.columns
        else 0
    )
    ...
    )  # StringIO já é texto; depois codificamos para utf-8
...
EMENDAS_CSV_URL = "https://docs.google.com/spreadsheets/d/1MwwFDqkmlXjtImorlUGoSaccADrkmrwUfiW6FvtcFsc/export?format=csv&gid=0"
...
def update_emendas_csv(request):
    ...
    try:
        with urlopen(req, timeout=30) as resp, open(tmp_path, "wb") as dest:
            ...
            if status != 200:
                return JsonResponse(
                    {"ok": False, "error": f"HTTP {status} ao baixar CSV"}, status=502
                )
        ...
        try:
            ...
            pass
        return JsonResponse(
            {"ok": False, "error": f"Falha ao atualizar: {e}"}, status=500
        )
